...
| `fixtures.ts` | Fail-loud fixture identity resolver (`E2E_FIXTURE_ADMIN_*` / `E2E_FIXTURE_EDITOR_*`); refuses the retired 0002 seed accounts — no fallback. Consumes identities provisioned by the backend `cmd/e2e-fixture-bootstrap` seam (backend ticket #19, currently a frontend release prerequisite) |
