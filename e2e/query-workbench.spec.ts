...
test.describe("Object Inspector metadata", () => {
  ...
  test("safe close when trigger button is unmounted by target change", async ({ page }) => {
    ...
    const triggerElement = await inspectButton.elementHandle();
    await inspectButton.click();

    const inspector = page.getByRole("dialog", { name: /schema_child — Inspector/ });
    await expect(inspector).toBeVisible();

    // Close inspector first (target-change effect fires on rerender regardless)
    await page.keyboard.press("Escape");
    await expect(inspector).toBeHidden();
    ...
    const options = getInactiveConnectionTargetButtons(page);
    const optionCount = await options.count();
    test.skip(optionCount < 1, "target-change inspector test needs >= 2 seeded targets");

    await options.first().click();
    await expect(getConnectionDialog(page)).toBeHidden();

    const isConnected = await triggerElement!.evaluate((el) => el.isConnected);
    expect(isConnected).toBe(false);
  });

  test("object collapse closes Inspector and clears trigger element", async ({ page }) => {
    await openQueryWorkbench(page);
    await ensureReadyTargetSelected(page);
    ...
    // Collapse the object via JS click (bypasses dialog focus trap overlay)
    await page.evaluate(() => {
      const items = document.querySelectorAll('[role="treeitem"][aria-expanded="true"]');
      for (const item of items) {
        const btn = item.querySelector('button[aria-expanded="true"]');
        if (btn && btn.textContent?.includes("schema_child")) {
    ...
    // Re-expand — Inspect button should reappear (trigger element state was cleared)
    await childTable.click();
    await expect(explorer.getByRole("button", { name: "Inspect" })).toBeVisible({ timeout: 10_000 });
  });
});
