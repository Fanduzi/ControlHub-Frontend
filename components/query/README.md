...
| query-editor-shell.tsx | Tabbed editor shell with server-authorized CSV export, serialized owner workspace hydration/OCC persistence, unavailable-target retargeting, worksheet limits across navigator actions, and server-authorized new-tab history restore. Worksheet session and result envelope live in `lib/` |
...
| query-object-explorer.tsx | Schema object explorer with tree and inspector; reads schema catalog, including database search, system visibility, and metadata refresh |
