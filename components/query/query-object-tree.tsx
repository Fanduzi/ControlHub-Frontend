          ...
          <li key={database} role="treeitem" aria-expanded={expanded}>
            ...
            {expanded ? (
              <ul role="group" className="ml-4 border-l border-border pl-2">
                {loadingDatabases.has(database) ? <li className="py-2 text-xs text-muted-foreground">Loading…</li> : null}
                {(["table", "view"] as const).map((kind) => {
                  const group = objects.filter((object) => object.kind === kind);
                  if (group.length === 0) return null;
                  const Icon = kind === "table" ? Table2 : View;
                  return (
                    <li key={kind} role="treeitem" aria-label={kind === "table" ? "Tables" : "Views"}>
                      <p className="flex items-center gap-1 px-2 py-1 text-xs font-medium text-muted-foreground"><Icon className="size-3" aria-hidden />{kind === "table" ? "Tables" : "Views"}</p>
                      <ul role="group">
                        {group.map((object) => {
                          ...
                          return (
                            <li key={key} role="treeitem" aria-expanded={objectExpanded}>
                              ...
                              {objectExpanded ? <div className="ml-5 py-1">{loadingObjects.has(key) ? <p className="text-xs text-muted-foreground">Loading…</p> : renderDetail(object)}</div> : null}
                            </li>
                          );
                        })}
                      </ul>
