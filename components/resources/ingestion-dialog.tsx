// input: native File input, React state, translations, ingestion service, shared dialog, and BFF API errors
// output: admin CSV/JSON ingestion preview, server-diff/conflict review, atomic confirmation, and result summary
// pos: resource-console entry point for server-owned ingestion review and confirmation
...
import { confirmIngestion, previewIngestion, type IngestionFormat, type IngestionPreview, type IngestionRelation, type IngestionValueDiff } from "@/services/resources";
...
function summary(preview: IngestionPreview): string {
  const counts = preview.rows.reduce((total, row) => ({ ...total, [row.action]: total[row.action] + 1 }), { create: 0, update: 0, conflict: 0 });
  return `${counts.create} created · ${counts.update} updated · ${counts.conflict} conflicts`;
}
