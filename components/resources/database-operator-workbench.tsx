    ...
    <div className="space-y-4">
      ...
      {!isCluster && (
        <div className="grid gap-4 xl:grid-cols-[1fr_1fr]">
          {clusterInfo && (
            <DetailPanel
              title={t("instanceContext.parentCluster")}
              description={t("instanceContext.parentClusterDescription")}
            >
              <div className="flex items-center gap-3">
                <div>
                  <Link
                    href={`/resources/${clusterInfo.id}`}
          ...
          {resource.profileSummary && (
            <DetailPanel
              title={t("instanceContext.connection")}
              description={t("instanceContext.connectionDescription")}
            >
              <dl className="grid gap-3 text-sm md:grid-cols-2">
                {resource.profileSummary.hostname && (
                  <div>
                    <dt className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                      {t("instanceContext.hostname")}
                    </dt>
                ...
                {resource.profileSummary.role && (
                  <div>
                    <dt className="text-xs uppercase tracking-[0.14em] text-muted-foreground">
                      {t("instanceContext.role")}
                    </dt>
                    <dd className="mt-1 font-medium text-foreground">
                      {t(`instanceContext.role_${resource.profileSummary.role.toLowerCase()}`) !==
                      `instanceContext.role_${resource.profileSummary.role.toLowerCase()}`
                        ? t(`instanceContext.role_${resource.profileSummary.role.toLowerCase()}`)
                        : resource.profileSummary.role}
                    </dd>
                  </div>
