...
const toneClasses: Record<NonNullable<StatusBadgeProps["tone"]>, string> = {
  ...
  lifecycle:
    "border-transparent bg-muted text-muted-foreground data-[status=running]:bg-primary/10 data-[status=running]:text-primary data-[status=provisioning]:bg-amber-500/10 data-[status=provisioning]:text-amber-700 dark:data-[status=provisioning]:text-amber-300 data-[status=degraded]:bg-orange-500/10 data-[status=degraded]:text-orange-700 dark:data-[status=degraded]:text-orange-300 data-[status=stopped]:bg-muted data-[status=stopped]:text-muted-foreground data-[status=decommissioning]:bg-muted/50 data-[status=decommissioning]:text-muted-foreground",
